...
const jsonToSQL = () =>{
    ...
    data.articles.forEach(article => { let content = JSON.stringify(article.content); console.log(content); console.log(typeof(content));sql += `INSERT INTO "Articles" ("id", "category", "title", "titleState", "author", "authorState", "time", "content", "createTime", "searchTimes", "userID") VALUES ('${article.id}', '${article.category}', '${article.title}', '${JSON.stringify(article.titleState)}', '${article.author}', '${JSON.stringify(article.authorState)}', '${article.time}', '${content.replaceAll("'","\\\"")}', '${article.createTime}', '${article.searchTimes}', '${article.userID}'); `})
    return sql
}
